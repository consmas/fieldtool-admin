  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <div>
        ...
        <h2 className="text-lg font-semibold md:text-xl">Trip Chats</h2>
      ...
      {isLoading ? (
        ...
        <>
          <div className="space-y-2 md:hidden">
            {threads.map((thread) => (
              <div key={thread.trip_id} className="ops-card p-3">
                <div className="flex items-start justify-between gap-2">
                  <p className="font-semibold text-foreground">
                    {thread.trip_reference ?? `Trip ${thread.trip_id}`}
                  </p>
                  {thread.unread_count > 0 ? (
                    <span className="rounded-full bg-rose-500/20 px-2 py-1 text-[11px] font-semibold text-rose-300">
                      {thread.unread_count}
                    </span>
                  ) : null}
                </div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Last message: {formatDate(thread.last_message_at ?? undefined)}
                </p>
                <Link
          ...
          <div className="ops-card hidden overflow-hidden md:block">
